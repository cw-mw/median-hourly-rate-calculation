...
import { Plus, Trash2, Calculator, Clock, LayoutList, UploadCloud, Loader2, Euro, Eye, EyeOff, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function App() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Set the file object to render in react-pdf
    setPdfFile(file);
    setPdfPage(1); // Reset to first page every time a new file is loaded
    ...
    try {
      ...
      if (data.items && Array.isArray(data.items)) {
        if (data.items.length === 0) {
          alert("We analyzed the PDF locally but couldn't detect any structured line items (OCR formatting mismatch). You may need to enter them manually, or we can re-enable the AI scanner for smarter extraction.");
        } else {
  ...
  return (
    <div className="min-h-screen p-4 sm:p-8 text-gray-900 bg-gray-50 flex justify-center">
      <div className="w-full max-w-7xl max-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {pdfFile && (
            <div className="flex-1 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden min-h-[500px] flex flex-col">
              <div className="bg-gray-100 p-3 border-b border-gray-200 flex items-center justify-between">
                <span className="text-sm font-semibold text-gray-600">Document Preview</span>
              ...
              <div className="flex-1 overflow-y-auto bg-gray-50 flex flex-col items-center p-4">
                <Document 
                  file={pdfFile} 
                  onLoadSuccess={({ numPages }) => setNumPdfPages(numPages)}
                  loading={<Loader2 className="w-6 h-6 animate-spin text-blue-500 my-8" />}
                  ...
                  {items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="col-span-12 sm:col-span-2 w-full flex items-center justify-between sm:justify-center">
